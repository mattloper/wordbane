...
How Wordplay is put together, for anyone changing more than the JSON in `data/`.
...
- **`web/`** — plain HTML/JS, emoji art, no build step. The shared, accessible
  build; the one meant to be played and hacked.
- **`godot/`** — a [Godot 4](https://godotengine.org/) project. The same game, plus
  a local AI-art pipeline that draws each monster (see [Art](#art-godot-only)). Run
  from source; it's the "deluxe" local build.

Both read the same **`data/`** and are kept honest by a shared **conformance
...
`res://../data/…` (which resolves to the repo root when run from source).

### Conformance: how drift is prevented

`data/conformance.json` is a language-neutral list of `input → expected` vectors
...
```bash
node web/test/conformance.js                                 # the JS core
godot --headless --path godot --script res://selftest.gd     # the Godot core (+ more)
```
...
| concept | web | godot |
|---|---|---|
| letter scoring + dictionary | `web/src/lexicon.js` | `godot/core/lexicon.gd` |
| one battle | `web/src/poolbattle.js` | `godot/core/pool_battle.gd` |
| enemy generation | `web/src/gauntlet.js` | `godot/core/gauntlet.gd` |
| rewards | `web/src/boons.js` | `godot/core/boons.gd` |
| RNG | `web/src/rng.js` | `godot/core/rng.gd` |
| tuning/catalog loader | `web/src/rules.js` | `godot/core/rules.gd` |

The view layers (`web/src/game.js`, `godot/pool_gauntlet.gd`) are separate and do
...
```
godot/  ──HTTP──▶  art/wordplay_art/server.py  ──gRPC──▶  Draw Things
                   (a tiny daemon; caches PNGs in .cache/)
```

`art/` is a [uv](https://docs.astral.sh/uv/) Python project. One endpoint,
...
## Generating the word lists (`tools/`)

`data/word_bank.json` and `data/dictionary.json` are committed, so you don't need
...
uv run wordplay-generate      # -> ../data/word_bank.json (curated creatures/items/adjectives)
uv run wordplay-dictionary    # -> ../data/dictionary.json (WordNet word set, filtered)
uv run pytest
```

The curated game vocabulary lives in `tools/wordplay_tools/lexicon.py`. Editing
`data/word_bank.json` directly is fine for quick mods, but a regenerate will
overwrite it — put lasting changes in `lexicon.py`.

## Testing

```bash
node web/test/conformance.js                                 # JS core vs golden vectors
godot --headless --path godot --script res://selftest.gd     # Godot core + conformance
godot --headless --path godot --script res://balance.gd      # difficulty-curve harness
cd tools && uv run pytest                                    # the generators
```

## Adding things

- **A monster/weapon:** add the word to the right pool in `data/word_bank.json` and
  an emoji in `data/icons.json`. (Both builds pick it up.)
- **A boon:** add it to `rules.json` `boons.catalog`, implement its effect in
  ...
  and an emoji in `icons.json` (+ a prompt in `art/wordplay_art/artwork.py` for the
  Godot art).
